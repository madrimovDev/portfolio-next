	...
	return (
		<div className="border-y border-line bg-surface2">
			<div className="mx-auto flex max-w-[1200px] flex-wrap items-center justify-between gap-[18px] px-5 py-6 sm:px-8">
				<span className="font-mono text-[11px] tracking-[.14em] text-faint">
					ISHONCH BILAN XIZMAT QILGAN SOHALAR
				</span>
				<div className="flex flex-wrap gap-9 font-display text-[clamp(15px,1.6vw,19px)] font-medium tracking-[.01em] text-soft">
					{SECTORS.map((s) => (
						<span key={s}>{s}</span>
					))}
				</div>
